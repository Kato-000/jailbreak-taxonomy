...
try:
    ...
    GCG_AVAILABLE = True
    print("[SUCCESS] GCG攻撃が利用可能です（real_gcg_with_models.py）")
except ImportError:
    try:
        from src.gcg_attack import GCGAttackWithEvaluation
        GCG_AVAILABLE = True
        print("[SUCCESS] GCG攻撃が利用可能です（src/real_gcg_with_models.py）")
    except ImportError:
        try:
            from gcg_attack import GCGAttackWithEvaluation
            GCG_AVAILABLE = True
...
def main():
    parser = argparse.ArgumentParser(
        description="バッチ攻撃実行システム - GCG, PAIR, AMJ対応"
    )
